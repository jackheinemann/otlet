...
class _CreateCustomToolScreenState extends State<CreateCustomToolScreen> {
  ...
  int customToolIndex = 0; // lets us navigate to fixed options page
  ...
  Widget build(BuildContext context) {
    return Consumer<OtletInstance>(
      builder: (context, instance, _) => WillPopScope(
        onWillPop: () async {
          bool onFixedOptions = customToolIndex == 1;
        ...
        child: customToolIndex == 0
                  ...
                  leading: IconButton(
                      ...
                      onPressed: () async {
                        bool shouldPop = await showConfirmDialog(
                            'Discard changes?', context);
                        if (shouldPop)
                          widget.updateScreenIndex(ScreenIndex.mainTabs);
                        return Future.value(false);
                      }),
                  title:
                      Text(isEdit ? 'Edit ${tool.name}' : 'Create Custom Tool'),
                  actions: [
                    if (isEdit)
                      IconButton(
                        ...
                        ElevatedButton(
                            ...
                            onPressed: () {
                              ...
                              if (tool?.isBookTool == null) {
                                showErrorDialog(
                                    context, 'You must select a target.');
                                return;
                              }
                              if (tool?.toolType == null) {
                                showErrorDialog(
            ...
            : CreateFixedOptionsScreen(Tool.fromTool(tool),
                updateCustomToolIndex: (index, {updatedTool}) {
                if (updatedTool != null) {
                  tool = updatedTool;
                }
                setState(() {
                  customToolIndex = index;
                  fixedOptionsController.text = tool.fixedOptions
                      .map((e) => e.toString())
                      .toList()
                      .join(', ');
                });
